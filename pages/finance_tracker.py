...
sidebar_main = st.sidebar.selectbox(
    "Navigation", ["Home", "See Finances", "Edit Monthly Income"]
)

if sidebar_main == "Home":
    st.title("Personal Finance Dashboard")
    st.subheader(
        "Please head over and set your monthly budget. (Ignore if already done)"
    )
    ...
    if submit:
        ...
        with open("pages/data/income.csv", mode="r") as file:
            ...
            for lines in csvFile:
                continue
        st.subheader(
            f"Your daily budget is : {round(float(lines[3])/(months[0] - int(day)),1)}"
        )
        st.subheader(
            f"The money you can spend this month is : {round(float(lines[3]),1)}"
        )
        if float(lines[3]) < 0:
    ...
    if check:
        st.write(
            "I used ",
            df.find_max("travel")[0],
            " on ",
            df.find_max("travel")[2].date(),
            " for ",
            df.find_max("travel")[1],
        )
